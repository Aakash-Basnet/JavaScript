...
for(let i=0;i<picArray.length;i++){
    ...
    let imgSrc=`${picArray[i].image.medium}`
    let alter=`${picArray[i].title}`

    articles.className="card"
    articles.innerHTML=`<h2>${picArray[i].title}</h2><figure><img src="${imgSrc}" alt="${alter}"><figcaption>${picArray[i].caption}</figcaption></figure><p>${picArray[i].description}</p>`
