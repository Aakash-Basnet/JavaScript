...
// add your code here
const pos=document.querySelector("#pictures")

const diag=document.querySelector("dialog")
const diagImg=diag.querySelector("img")
const spanClose=diag.querySelector("span")

spanClose.addEventListener("click",()=>{diag.close()})
...
for(let i=0;i<picArray.length;i++){
    ...
    figure1.appendChild(figCap)
    let p=document.createElement("p")
    p.innerText=picArray[i].description
    articles.appendChild(p)
    pos.appendChild(articles)
    articles.addEventListener("click",()=>{
    diagImg.src=picArray[i].image.large
    diagImg.alt=picArray[i].title
    diag.showModal();
    })
}
